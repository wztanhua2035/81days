(() => {
  ...
  const SAVE = 'day81_save_v2', ACH = 'day81_achievements_v1';
  const app = document.getElementById('app');
  const DEFAULT_GAME_CONFIG = {
    roleNames: {linlan:'林岚',zhouye:'周野',chenmo:'陈默',suqing:'苏晴',gaoyuan:'高远',xutang:'许棠'},
    difficulty: {nightEventChance:.50,baseCheckModifier:0,encounterInterval:5,avoidChance:.20,healthDecayChance:1,healthyLifeRecoverChance:.20,inventoryLimit:4,startingBonusFood:0}
  };
  ...
  function rng(){ let a=state.rngState>>>0; a=(a+0x6D2B79F5)>>>0; state.rngState=a; let t=a; t=Math.imul(t^(t>>>15),t|1); t^=t+Math.imul(t^(t>>>7),t|61); return ((t^(t>>>14))>>>0)/4294967296; }
  ...
  function load(){ try{ const x = JSON.parse(localStorage.getItem(SAVE)||'null'); if(x && x.version===2){ x.rules={...DEFAULT_GAME_CONFIG.difficulty,...(x.rules||GAME_CONFIG.difficulty)}; x.exploredLocations=x.exploredLocations||{}; x.pendingEndDay=!!x.pendingEndDay; applyNamesToCharacters(x.characters); x.characters.forEach(c=>c.inventoryLimit=x.rules.inventoryLimit); return x; } }catch(e){} return null; }
  ...
  function makeChar(base){
    return {...base,life:base.maxLife,health:3,inventory:[base.startItem],dead:false,deathDay:null,deathCause:'',healthyStreak:0,abilityCooldown:0,lowLifeSeen:false,skipDecay:false,damageShield:0,generalShield:0,nextCheckBonus:0,dayStartHealth:3,raincoatUsed:false,lifevestUsed:false,stats:{battles:0,wins:0,trades:0,avoids:0,itemsFound:1,itemsUsed:0}};
  }
  function newState(playerId){
    const seed=`${Date.now()}-${Math.random()}`;
    const currentRules={...DEFAULT_GAME_CONFIG.difficulty,...GAME_CONFIG.difficulty};
    const s={version:2,seed,rngState:hashSeed(seed),day:1,phase:'PREPARE',playerId,characters:D.characters.map(makeChar),rules:currentRules,relationships:{},freeTrades:{},recentEvents:[],currentEvent:null,eventResolved:false,currentResult:'',encounterDoneDay:0,encounter:null,nightDanger:0,rescueScore:0,history:[],statistics:{battles:0,battleWins:0,trades:0,itemsUsed:0,itemsFound:0,foodUsed:0,risksTaken:0},pending:null,lastNight:null,score:null,selectedLocationId:null,exploredLocations:{},pendingEndDay:false};
  ...
  function useItem(c,id){
    const idx=c.inventory.indexOf(id); if(idx<0) return; const it=item(id); if(!it.consumable) return;
  ...
  function checkChance(c,stat,difficulty='normal',tags=[]){
    ...
    let p=base+(difficulty==='easy'?.15:difficulty==='hard'?-0.20:0)+(c.nextCheckBonus||0)+(rules().baseCheckModifier||0);
  ...
  function applyEffect(c,effect={},context='事件'){
    ...
    if(effect.nextCheckBonus){ c.nextCheckBonus=Math.max(c.nextCheckBonus,effect.nextCheckBonus); msgs.push('你记下了更多线索'); }
    ...
    if(effect.loseItem){ const lost=loseRandomItem(c); msgs.push(lost?`失去${item(lost).name}`:'没有丢失任何道具'); }
  ...
  function chooseLocationEvent(locationId){
    let pool=D.locationEvents.filter(e=>e.location===locationId && (!e.minDay || state.day>=e.minDay) && !state.recentEvents.includes(e.id));
  ...
  function exploreLocation(locationId){
    if(state.currentEvent||state.eventResolved) return;
    const loc=locationById(locationId); if(!loc) return;
    state.selectedLocationId=locationId;
    state.exploredLocations[locationId]=true;
    state.currentEvent=chooseLocationEvent(locationId);
    state.phase='EVENT';
    log(`你前往${loc.name}探索。`);
    save(); render();
  }
  ...
  function resolveInstantOrCheck(index=null){
    ...
    else if(e.type==='check'){ const ok=checkChance(c,e.stat,e.difficulty,e.tags||[]); result=ok?`检定成功。${applyEffect(c,e.success,e.name)}`:`检定失败。${applyEffect(c,e.fail,e.name)}`; }
  ...
  function resolveChoice(c,e,ch,isNpc){
    ...
    if(ch.action==='effect') return applyEffect(c,ch.effect||{},e.name) || '无事发生';
    if(ch.action==='check'){
      const ok=checkChance(c,ch.stat,ch.difficulty,ch.tags||e.tags||[]);
      if(ok) return `检定成功。${applyEffect(c,ch.success,e.name)}`;
      const f=ch.fail||{};
      if(f.chance && !chance(f.chance)) return '检定失败，但没有造成进一步损失。';
      if(f.beast) return `检定失败。${beastHazard(c)}`;
      return `检定失败。${applyEffect(c,f,e.name)}`;
    }
  ...
  function startDay(){
    ...
    state.phase='PREPARE'; state.currentEvent=null; state.eventResolved=false; state.currentResult=''; state.nightDanger=0; state.selectedLocationId=null; state.pendingEndDay=false; save(); render();
  }
  ...
  function beginEncounter(silent=false){
    const a=alive(); shuffle(a); const pair=a.slice(0,2); state.encounter={a:pair[0].id,b:pair[1].id,result:'',resolved:false};
    if(!pair.some(c=>c.id===state.playerId)){ resolveNpcEncounter(pair[0],pair[1]); state.encounterDoneDay=state.day; state.encounter=null; if(!silent){ save(); render(); } return false; }
    state.phase='ENCOUNTER'; if(!silent){ save(); render(); } return true;
  }
  function encounterAction(action,giveId=null){
    ...
    else if(action==='avoid'){ p.stats.avoids++; const baseAvoid=Number(rules().avoidChance)||.20; const success=chance(clamp(baseAvoid+(p.id==='suqing'?.15:0),.05,.95)); if(success) result='你成功避开了对方。'; else if(enemy(p,o)||chance(.5)) result=`躲避失败。${fight(p,o)}`; else result=`躲避失败。${trade(p,o,p.inventory[0]||null)}`; }
  ...
  function closeEncounter(){
    ...
    if(state.pendingEndDay){ state.pendingEndDay=false; continueNightPhase(); return; }
    state.phase='PREPARE'; save(); render();
  }
  ...
  function endDay(){
    const p=player(); if(p.dead) return;
    if(dueEncounter() && state.encounterDoneDay!==state.day){ state.pendingEndDay=true; const needs=beginEncounter(true); if(needs){ save(); render(); return; } }
    continueNightPhase();
  }
  function runNight(){
  ...
  function settleDay(){
    ...
    state.currentEvent=null; state.eventResolved=false; state.currentResult=''; state.selectedLocationId=null;
  }
  ...
  function endGame(win){
    const p=player(); state.phase=win?'VICTORY':'GAME_OVER';
    if(win){ unlock('day81'); if(alive().length===6) unlock('all_alive'); if(state.statistics.battles===0) unlock('pacifist'); if(state.statistics.battles>=5) unlock('island_boss'); }
    const survived=win?81:Math.max(1,state.day); let score=Math.round((survived/81)*50)+(win?20:0);
    score+=Math.round(10*((p.life/p.maxLife)*.7+(p.health/3)*.3));
    score+=Math.min(8,Math.round((p.inventory.reduce((s,id)=>s+(item(id).value||0),0)/32)*8));
    score+=Math.max(0,Math.min(7,Math.round(state.statistics.trades*1.5-state.statistics.battles*.6+3)));
    score+=Math.min(5,Math.round(state.rescueScore));
  ...
  function mapHtml(){
    const exploredCount=Object.keys(state.exploredLocations||{}).length;
    return `<section class="card islandCard"><div class="row between"><div><div class="section-title">海岛地图</div><div class="name" style="font-size:18px">清晨探索 · 12个地点</div></div><div class="meta">已探索 ${exploredCount}/12</div></div><div class="islandMap"><div class="islandSea"></div><div class="islandBody"></div>${D.locations.map(loc=>{ const known=state.exploredLocations?.[loc.id]; const label=known?loc.name:'未知'; const cls=known?'known':''; return `<button class="mapSpot ${cls}" style="left:${loc.x}%;top:${loc.y}%" onclick="Game.explore('${loc.id}')"><span class="pin">${known?loc.icon:'❓'}</span><span class="spotLabel">${esc(label)}</span></button>`; }).join('')}</div><div class="mapLegend">点击任意地点开始今天的探索。未探索地点会显示为“未知”。${dueEncounter()?'<br><span class="muted">提示：今天傍晚你可能会与其他幸存者发生遭遇。</span>':''}</div></section>`;
  }
  ...
  function renderMain(){
    ...
    if(state.phase==='EVENT'&&state.currentEvent){
      const e=state.currentEvent, loc=locationById(state.selectedLocationId);
      let controls='';
      if(e.type==='choice') controls=e.choices.map((ch,i)=>`<button class="choice" onclick="Game.resolve(${i})">${esc(ch.text)}<small>${esc(riskHint(ch))}</small></button>`).join('');
      else controls=`<button class="btn" onclick="Game.resolve()">执行</button>`;
      center=`<section class="paper"><div class="section-title">今日探索 · ${esc(loc?.name||'未知地点')}</div><div class="event-title">${esc(e.name)}</div><div class="event-text">${esc(e.text)}</div><div class="choices">${controls}</div></section>`;
    } else if(state.phase==='POST'){
      const loc=locationById(state.selectedLocationId);
      center=`<section class="paper"><div class="section-title">探索结果 · ${esc(loc?.name||'未知地点')}</div><div class="event-title">${esc(state.currentEvent?.name||'今日探索')}</div><div class="event-text">${esc(state.currentEvent?.text||'')}</div><div class="result">${esc(state.currentResult)}</div></section><button class="btn" onclick="Game.endDay()">结束今天</button>`;
    } else {
      center=`${mapHtml()}<section class="paper"><div class="section-title">今日安排</div><div class="event-title">选择一个地点探索</div><div class="event-text">每天清晨，你都要先在海岛上选定一个地点行动。到达后会触发与该地点相关的随机事件，你需要在三个选项中做出选择。探索结束后，可以继续整理背包，再结束今天。</div></section>`;
    }
  ...
  function renderEncounter(){ const p=player(), o=cBy(state.encounter.a===p.id?state.encounter.b:state.encounter.a), en=enemy(p,o), e=state.encounter; if(e.resolved){ app.innerHTML=`<div class="screen">${topHtml()}${statusHtml(p)}<section class="paper"><div class="section-title">人物遭遇</div><div class="event-title">${o.avatar} ${o.name}</div><div class="event-text">${esc(e.result)}</div></section><button class="btn" onclick="Game.closeEncounter()">${state.pendingEndDay?'进入夜晚':'继续今天'}</button></div>`; return; } const diff=combatPower(p)-combatPower(o), adv=diff>0?'优势':diff<0?'劣势':'势均力敌'; let tradeBtn=en?'':`<button class="btn secondary" onclick="Game.tradeModal()">交易</button>`; app.innerHTML=`<div class="screen">${topHtml()}${statusHtml(p)}<section class="paper"><div class="section-title">黄昏遭遇</div><div class="event-title">${o.avatar} ${o.name} · ${o.job}</div><div class="event-text">生命 ${hearts(o)}\n健康 ${healthDots(o)}\n力量 ${o.str} · 敏捷 ${o.agi}\n背包 ${o.inventory.length}件\n关系：${en?'敌人':'非敌对'}\n战斗判断：${adv}</div></section><div class="choices"><button class="btn danger" onclick="Game.encounter('fight')">战斗</button><button class="btn secondary" onclick="Game.encounter('avoid')">躲避</button>${tradeBtn}</div></div>`; }
  ...
  window.Game={
    ...
    explore:exploreLocation, resolve:resolveInstantOrCheck, endDay,
    encounter:encounterAction, closeEncounter,
